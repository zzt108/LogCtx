...
using GreenSpeed.ATO.Services.Logging;
using Newtonsoft.Json;
using NLog;

namespace Ato.SpecFlow.Shared.Support
{
    public static class LogCtx
    {
        ...
        public static Logger? Logger = CanLog ? LogManager.GetCurrentClassLogger() : null;

        public static void InitLogCtx(string confignlogconfig)
        {
                var loggingService = new LoggingService();
                loggingService.Init(confignlogconfig);
            ...
            [CallerLineNumber] int sourceLineNumber = 0)
        {
            ...
            ScopeContext.PushProperty(STRACE, strace2);
            ...
            {
                ScopeContext.PushProperty(key, scopeContextProps[key]?.ToString());
            }
